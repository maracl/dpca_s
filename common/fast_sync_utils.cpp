...
void fast_rwl_init(fast_rwlock* p) {
	//p->_res[0]=0;
	//p->_res[1]=0;
	fast_mutex_init(&p->wlock);
	fast_mutex_init(&p->rlock);
	p->rcount = 0;
}

void fast_rwl_rlock(fast_rwlock* p) {
	fast_mutex_lock(&p->rlock);
	if (p->rcount++ == 0) {
		fast_mutex_lock(&p->wlock);
	}
	fast_mutex_unlock(&p->rlock);
}

void fast_rwl_runlock(fast_rwlock* p) {
	fast_mutex_lock(&p->rlock);
	if (--p->rcount == 0) {
		fast_mutex_unlock(&p->wlock);
	}
	fast_mutex_unlock(&p->rlock);
}

void fast_rwl_wlock(fast_rwlock* p) {
	fast_mutex_lock((fast_mutex*)&p->wlock);
}

void fast_rwl_wunlock(fast_rwlock* p) {
	fast_mutex_unlock((fast_mutex*)&p->wlock);
}
