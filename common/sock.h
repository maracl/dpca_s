...
inline int socket_bind_dev(IN socket_t sock,  IN const char* dev)
{
#if defined(OS_WINDOWS)
	return 0;
#else
...
inline int socket_select_read(IN socket_t sock, IN int timeout)
{
	...
	return socket_select_readfds((int)sock+1, &fds, timeout<0?NULL:&tv);
#else
	int r;
	struct pollfd fds;
...
inline int socket_select_write(IN socket_t sock, IN int timeout)
{
	...
	tv.tv_sec = timeout/1000;
	tv.tv_usec = (timeout%1000) * 1000;
	return socket_select_writefds((int)sock+1, &fds, timeout<0?NULL:&tv);
#else
	int r;
	struct pollfd fds;
