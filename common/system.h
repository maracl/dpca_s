...
inline void system_getmeminfo(uint64_t &total, uint64_t &avail)
{
	static int usage = 0;
...
inline long long system_timestamp(void)
{
	...
	return ((ns100 / 10LL) % 1000000LL)/1000+
		((ns100 - 116444736000000000LL) / 10000000LL)*1000;
#endif
}
...
inline int system_rand(int64_t a =0, int64_t b = 0)
{
#ifdef _WIN32
