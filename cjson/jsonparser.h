...
public:
	...
	bool get(const char* xpath, long long& value) const
	{
		const json_value* json = find(xpath);
		return json ? get(json, value) : false;
	}
...
public:
	...
	bool get(const json_value* json, long long& value) const
	{
		if(json->type == json_string)
		{
			value = _atoi64(json->u.string.ptr);
		}
		else
		{
			assert(json->type == json_integer);
	...
	{
		assert(json->type == json_string || json->type == json_null);
		if(json->type != json_string)
			return false;
